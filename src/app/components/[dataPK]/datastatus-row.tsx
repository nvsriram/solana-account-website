import { ClusterNames, DataStatusOption, IDataAccountMeta } from "@/app/utils/types";
...
const DataStatusRow = ({ dataPK, meta, refresh } : { dataPK: string | undefined, meta : IDataAccountMeta, refresh: () => void }) => {
    ...
    const isAuthority = useMemo(() => authority && authority.toBase58() === meta.authority, [authority, meta]);
    ...
    const handleFinalize = async () => {
        ...
        if (!authority || meta.authority != authority.toBase58() || !signTransaction) {
        ...
        try {
            ...
            const txid = await clusterConnection.sendRawTransaction(signed.serialize());
            await clusterConnection.confirmTransaction(
                ...
                console.log(`finalized: https://explorer.solana.com/tx/${txid}?cluster=devnet`);
            });
    ...
    return (
        <tr>
            ...
            <td className={`text-md flex items-center ${meta.data_status === DataStatusOption.INITIALIZED ? "text-solana-green": "text-rose-500"}`}>
                ...
                {meta.data_status != DataStatusOption.FINALIZED && 
                    <>
                        ...
                        <Tooltip message={<>{isAuthority ? "Finalized accounts can no longer be updated" : "Login as Authority wallet to finalize data account"}</>} condition={true} sx={`w-44 right-0 top-0 left-9`}>
                        ...
                        {error && 
                            <p className="ml-5 text-rose-500">{error}</p>}
                    </>}
            </td>
        </tr>
    );
