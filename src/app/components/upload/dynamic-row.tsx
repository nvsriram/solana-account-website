...
const DynamicRow = ({ isDynamic, setIsDynamic, space, setSpace } : { isDynamic: boolean, setIsDynamic: Dispatch<SetStateAction<boolean>>, space: number, setSpace: Dispatch<SetStateAction<number>> }) => {
    return (
        <tr>
            <th
            ...
            <td className="flex flex-row h-full items-center">
                ...
                <Tooltip message={<><b>{isDynamic ? "Dynamic" : "Static"}</b><br />Initial size:<br />{displaySize(space)}<br />{space > MAX_FILE_SIZE ? <p className="text-rose-700">{`> ${MAX_FILE_SIZE/1e6} MB`}</p> : null}</>} condition={true} sx={`w-32 right-0 top-0 left-9`}>
