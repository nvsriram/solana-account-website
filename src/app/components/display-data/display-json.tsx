...
const JSONDisplay = ({ json, len, dataPK, meta, refresh }: { json: object, len: number, dataPK: string, meta: IDataAccountMeta, refresh: () => void }) => {
    ...
    const unsavedChanges = useMemo(() => JSON.stringify(data) != JSON.stringify(json), [data, json]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (old.length === updated.length) {
                ...
                updateData = Buffer.concat([oldUpdate, Buffer.from(new Uint8Array(old.length - updated.length))]);
            }
